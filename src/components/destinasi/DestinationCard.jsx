...
export default function DestinationCard({ destination, variant = 'compact', className = '' }) {
  const isCompact = variant === 'compact';

  const getShadowClass = (category) => {
    switch (category?.toLowerCase()) {
      case 'alam': return 'group-hover:shadow-colored-teal';
      case 'budaya': return 'group-hover:shadow-colored-gold';
      case 'pantai': return 'group-hover:shadow-colored-blue';
      default: return 'group-hover:shadow-soft';
    }
  };

  return (
    <Link to={`/destinasi/${destination.id}`} className={`block group ${className}`}>
      <div className={`relative rounded-2xl overflow-hidden bg-surface border border-surface-alt transition-all duration-300 transform group-hover:-translate-y-1 ${getShadowClass(destination.category)} shadow-sm`}>
        <LazyImage 
          ...
          className={isCompact ? "aspect-[4/3]" : "aspect-video"} 
        />
