...
import KisahDestinasiPanel from './components/KisahDestinasiPanel';
import LazyImage from '../../components/common/LazyImage';
import Skeleton from '../../components/ui/Skeleton';
import RatingBadge from '../../components/ui/RatingBadge';
import Button from '../../components/ui/Button';
import Chip from '../../components/ui/Chip';
import { ArrowLeft, Clock, MapPin, Tag, Navigation, BookmarkPlus, CheckCircle2, Ticket, Tent } from 'lucide-react';
  ...
  if (!dest) {
    return (
      <div className="p-8 text-center mt-20">
        ...
        <Button variant="secondary" className="mt-4" onClick={() => navigate(-1)}>Kembali</Button>
      </div>
    );
  }
  ...
  return (
    <PageTransition>
      ...
      <div className="relative w-full h-72 md:h-96">
        <button 
          onClick={() => navigate(-1)} 
          className="absolute top-safe mt-4 left-4 z-20 w-10 h-10 rounded-full bg-white/50 backdrop-blur flex items-center justify-center text-ink border border-white/50 shadow-soft"
        >
          <ArrowLeft size={20} />
        </button>
        <LazyImage 
      ...
      <div className="relative z-10 -mt-12 px-6 pb-24">
        ...
        <div className="bg-surface rounded-2xl p-5 border border-surface-alt grid grid-cols-2 gap-y-5 gap-x-4 shadow-soft mb-6">
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="text-sm font-medium text-ink capitalize">Rp {dest.ticketPrice?.toLocaleString('id-ID') || 'Gratis'}</span>
          </div>
          <div className="flex flex-col gap-1">
        ...
        {dest.facilities && (
          <div className="mb-6">
            <h3 className="text-sm font-semibold text-ink mb-3">Fasilitas Tersedia</h3>
            <div className="flex flex-wrap gap-2">
              {dest.facilities.map(fac => (
                <Chip key={fac} label={fac} className="bg-base border-none" />
              ))}
            </div>
          </div>
        )}
